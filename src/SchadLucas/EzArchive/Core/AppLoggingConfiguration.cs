...
namespace SchadLucas.EzArchive.Core
{
    internal class AppLoggingConfiguration : LoggingConfiguration
    {
        public AppLoggingConfiguration()
        {
            ...
            {
                Layout = "${processtime:invariant=true}|${pad:padding=-30:inner=[${level:format=FirstCharacter}\\:${logger}]} ${message} ${onexception:inner=${newline}${exception:format=toString,Data:maxInnerExceptionLevel=10}}",
                Name = "Developer Log"
            };
